...
<script>
	export default{
		...
		mounted() {
			...
			//获取用户要查询的博客关键字
			this.inputTitle = this.$router.history.current.params.bloglike;
		...
		methods:{
			...
			likeTitle(){
				...
				this.$ajax.post('/api/blog/likeTitleList', this.$qs.stringify(data))
				.then(response =>{
					if(!response.data){
						this.inputResultNull = true;
					}
					else{
						this.inputResultNull = false;
