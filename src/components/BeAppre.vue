					...
					<ul class="nav navbar-nav navbar-right">
						...
						<li class="dropdown"  v-if="isLogin">
							...
							<ul class="dropdown-menu" style="text-align: left; padding-left: 20px;">
								<li>
									<!-- <router-link :to="`myBlogCenter${unread.pubusers.id}`">公告
		...
		<div class="m-card-right ">
			<div>
				...
				<div class="my-title-bottom">
					<ul>
						<li class="item-follow" v-for="item in appreList">
							<p style="line-height: 18px;">
								<span class="appre-uname">
									<router-link  :to="`myBlogCenter${item.sendUserId}`">{{item.username}}</router-link>
									<!-- <a href="" @click="gotoBlogCenter(item.sendUserId)" class="appre-head">{{item.username}}</a> -->
									
									<span class="attentime">点赞了你的博文</span>
								</span>
								<span class="attentime" >{{item.sendTime | formatDateyy}}</span>
								<a href="#" @click="deleteNotice(item.id)">
									<span class="glyphicon glyphicon-remove" aria-hidden="true" style="color: #B22222; margin-right: 8px; float: right;"></span>
									
								</a>
							</p>
							
							<p class="appre-title">
								<!-- <router-link :to="`myBlogCenter${item.targetId}`">{{item.title}}</router-link> -->
								<a href="#" @click="gotoBlogDetail(item.targetId)">{{item.title}}</a>
							</p>
							
							<!-- <a href="#"  class="fol-name" >他回国后</a>
							<span class="buttright" >关注了你</span>
							<span class="attentime" >2020-10-09</span>
							<a href="#" >
								<span class="glyphicon glyphicon-remove" aria-hidden="true" style="color: #B22222; margin-right: 8px; float: right;margin-top: 33px;"></span>
	...
	export default{
		data(){
			return{
				...
				unread:{},	//未读的消息列表
				appreList:{},	//点赞集合（包括username，title）
		...
		methods:{
			...
			getAppreList(){
				...
				this.$ajax.post('/api/notice/titleUnameNoticeList', this.$qs.stringify(data))
				.then(response =>{
					this.appreList = response.data;
				})
				.catch(error =>{
					console.log(error);
				})
